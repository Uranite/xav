...
Write-Host "  2. HIP"
Write-Host "  3. Vulkan"
$vshipChoice = Read-Host "Enter choice (1-3) [Default: 3]"
if (-not $vshipChoice) { $vshipChoice = '3' }
...
if (-not $hasCppTools) {
    ...
    # --wait is only valid for the bootstrapper, not for vs_installer.exe modify
    $vsWorkloadComponents = "--add Microsoft.VisualStudio.Workload.VCTools --add Microsoft.VisualStudio.Component.VC.Tools.x86.x64 --add Microsoft.VisualStudio.Component.Windows11SDK.22621 --includeRecommended"
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        # Script is already elevated, so vs_installer.exe runs elevated automatically.
...
# This block runs BEFORE vcvars64, so VCPKG_ROOT still reflects the user's real
# setting (vcvars64 later overwrites it with the VS-bundled manifest-only vcpkg).
...
# Pin to the resolved standalone install so vcvars64 cannot clobber it later
...
function Invoke-WithMsvc {
    ...
    # VCPKG_ROOT is re-pinned afterwards since vcvars64 overwrites it.
    $vcvarsCmd = if ($vshipBackend -eq 'cuda' -and $vcvarsArg) {
        Write-Host "[INFO] Forcing v143 toolset for CUDA compatibility..." -ForegroundColor Cyan
        "`"$vcvarsScript`" $vcvarsArg"
    } else {
        "`"$vcvarsScript`""
    }
        ...
        'hip' {
            ...
            Invoke-Step "Compiling Vship (HIP)" {
                & "$env:HIP_PATH\bin\hipcc" -c src/VshipLib.cpp -std=c++17 -I include `
                    --offload-arch=gfx1201,gfx1200,gfx1100,gfx1101,gfx1102,gfx1103,gfx1151,gfx1012,gfx1030,gfx1031,gfx1032,gfx906,gfx801,gfx802,gfx803 `
...
if (Test-Path 'lib\opus.lib') {
    ...
    Invoke-Step "Configuring Opus" {
        cmake --fresh -B build -G Ninja `
            ...
            -DCMAKE_C_FLAGS_RELEASE="-flto -O3 -DNDEBUG" `
...
if (Test-Path 'ffms2\lib\ffms2.lib') {
    ...
    Invoke-Step "Configuring FFMS2" {
        cmake --fresh -B ffms2_build -G Ninja -DBUILD_SHARED_LIBS=OFF -DENABLE_AVISYNTH=OFF `
            -DCMAKE_BUILD_TYPE=Release `
            -DCMAKE_CXX_FLAGS_RELEASE="-flto -O3 -DNDEBUG" `
            -DCMAKE_C_FLAGS_RELEASE="-flto -O3 -DNDEBUG"
    }
...
if (Test-Path 'lib\SvtAv1Enc.lib') {
    Write-Host "[INFO] svt-av1-hdr already compiled. Skipping..." -ForegroundColor Cyan
} else {
    if (Test-Path 'svt-av1-hdr') { Push-Location svt-av1-hdr; git pull; Pop-Location }
    else { git clone --depth 300 https://github.com/juliobbv-p/svt-av1-hdr.git }
    Push-Location svt-av1-hdr
    Invoke-Step "Configuring svt-av1-hdr" {
        cmake --fresh -B svt_build -G Ninja -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=OFF `
            -DSVT_AV1_LTO=OFF -DLIBDOVI_FOUND=0 -DLIBHDR10PLUS_RS_FOUND=0 -DENABLE_AVX512=ON `
            -DCMAKE_CXX_FLAGS_RELEASE="-flto -DNDEBUG -O2 -march=znver2" `
            -DCMAKE_C_FLAGS_RELEASE="-flto -DNDEBUG -O2 -march=znver2" `
            -DLOG_QUIET=ON -DCMAKE_MSVC_RUNTIME_LIBRARY=MultiThreaded
    }
    Invoke-Step "Building svt-av1-hdr" { ninja -C svt_build }
    Pop-Location
    if (-not (Test-Path 'lib')) { New-Item -ItemType Directory 'lib' | Out-Null }
    Copy-Item svt-av1-hdr\Bin\Release\SvtAv1Enc.lib lib\ -Force
}
...
switch ($vshipBackend) {
    'cuda' {
        Invoke-Step "Cargo build (CUDA)" {
            cargo build --release --features "static,vship,nvidia,vcpkg,libsvtav1" --config $cargoConfig
        }
    }
    'hip' {
        Invoke-Step "Cargo build (HIP)" {
            cargo build --release --features "static,vship,amd,vcpkg,libsvtav1" --config $cargoConfig
        }
    }
    'vulkan' {
        Invoke-Step "Cargo build (Vulkan)" {
            cargo build --release --features "static,vship,vcpkg,libsvtav1" --config $cargoConfig
        }
    }
}
